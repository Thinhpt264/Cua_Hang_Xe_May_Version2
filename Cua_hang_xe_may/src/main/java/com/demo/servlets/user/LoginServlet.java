...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost_Login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(accountModel.checkLogin(username, password)) {
			...
			logModel.create(new Log(IPAddressUtil.getPublicIPAddress(),"info" , ConfigIP.ipconfig(request).getCountryLong(), new Timestamp(new Date().getTime()), "Đăng Nhập Thành Công", null, null , account.getId() ) );
